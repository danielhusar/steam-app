...
module.exports = function (app) {
	app.factory('ItemDataFactory', function(SanitizeFactory) {
			...
			return function ($item) {
				...
				var $priceNoFee = $item.find('.market_listing_price_without_fee').length ? $item.find('.market_listing_price_without_fee') : $item.find('.market_table_value span');
				item.priceNoFee = $priceNoFee.text().replace(/($|€|£|USD)/gi, '').replace(/\-/gi, '0').replace(',', '.').trim();

				var $priceFee = $item.find('.market_listing_price_with_fee').length ? $item.find('.market_listing_price_with_fee') : $item.find('.market_table_value span');
				item.priceFee = $priceFee.text().replace(/($|€|£|USD)/gi, '').replace(/\-/gi, '0').replace(',', '.').trim();

				// item.id = ($item.html().match(/listing\_sell\_new\_(.*)\_image/) || $item.html().match(/resultlink\_(.*)\"/))[1];
